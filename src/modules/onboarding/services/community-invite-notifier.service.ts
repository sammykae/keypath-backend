...
interface SendCommunityInviteEmailInput {
  toEmail: string;
  firstName: string;
...
export async function sendCommunityOnboardingInviteEmail(
  input: SendCommunityInviteEmailInput
  ...
  const subject = 'Your KeyPath community onboarding link';

  const text = [
    `Hi ${input.firstName},`,
    '',
    'You have been invited to complete your community onboarding on KeyPath.',
  ...
  const html = `<!DOCTYPE html><html><body style="margin:0;padding:32px 16px;background:#e8eef2;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;">
    <div style="max-width:480px;margin:0 auto;background:#fff;border-radius:16px;padding:32px;border:1px solid #e2e8f0;">
      ...
      <h2 style="margin:16px 0 8px;font-size:22px;color:#0f172a;">Community onboarding invitation</h2>
      <p style="color:#64748b;margin:0 0 24px;">Hi ${e(input.firstName)}, you have been invited to complete your community onboarding on KeyPath.</p>
  ...
  if (!isEmailReady()) {
    logger.warn({ toEmail: input.toEmail }, 'SendGrid not configured — community invite not sent');
    return { messageId: 'not-sent' };
  }

  await sendEmail({ to: input.toEmail.trim().toLowerCase(), subject, text, html });

  logger.info({ toEmail: input.toEmail }, 'Community onboarding invite email sent via SendGrid');
