...
/** POST /api/landlord/agreements/upload — multipart file upload, returns fileKey to reference in the create/upload-agreement call. */
export async function agreementUploadHandler(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    const key = `agreements/${req.auth._id.toString()}/${crypto.randomUUID()}_${safeName}`;
